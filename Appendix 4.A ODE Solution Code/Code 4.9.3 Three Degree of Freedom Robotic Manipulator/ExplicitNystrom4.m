function [vn,vdn,vddn,Mcond]=ExplicitNystrom4(tnm,vnm,vdnm,h,par,integ)

%Nystrom integration step for v and vd, plus evaluation of vdd
f=ODEfunct(tnm,vnm,vdnm,par,dat,integ);
k1=f;
f=ODEfunct(tnm+h/2,vnm+(h/2)*vdnm+((h^2)/8)*k1,vdnm+(h/2)*k1,par,dat,integ);
k2=f;
f=ODEfunct(tnm+h/2,vnm+(h/2)*vdnm+((h^2)/8)*k1,vdnm+(h/2)*k2,par,dat,integ);
k3=f;
f=ODEfunct(tnm+h,vnm+h*vdnm+((h^2)/2)*k3,vdnm+h*k3,par,dat,integ);
k4=f;
