function [vn,vdn,vddn,Mcond]=ExplicitRungeKutta4(tnm,vnm,vdnm,h,par,dat)

%RungeKutta4 integration step for v and vd plus evaluation of vdd
f=ODEfunct(tnm,vnm,vdnm,par,dat);
k1=f;
f=ODEfunct(tnm+h/2,vnm+(h/2)*vdnm,vdnm+(h/2)*k1,par,dat);
k2=f;
f=ODEfunct(tnm+h/2,vnm+(h/2)*vdnm+((h^2)/4)*k1,vdnm+(h/2)*k2,par,dat);
k3=f;
f=ODEfunct(tnm+h,vnm+h*vdnm+((h^2)/2)*k2,vdnm+h*k3,par,dat);
k4=f;
