function QAsLam=QAsLamEval(tn,q,qd,Lam,SJDT,STSDAT,par)
...
QAsLam=Add(QAsLam,Q1krevfrsLam,7*(i-1),ms-1);

if j>=1
Q2krevfrsLam=[zeros(3,1);-2*Ep2'*A1*vz1pr]*...
    (tau12prkrev5frsLam+tau12kcylfrsLam);
QAsLam=Add(QAsLam,Q2krevfrsLam,7*(j-1),ms-1);
end
end
...
QAsLam=Add(QAsLam,Q1ktranfrsLam,7*(i-1),ms-1);

if j>=1
Q2ktranfrsLam=[-A1*vz1pr;-BT2'*A1*vz1pr]*f12ktranfrsLam;  
QAsLam=Add(QAsLam,Q2ktranfrsLam,7*(j-1),ms-1);    
end
