...
%Jacobian Evaluation
[Rvdd,Rvd,Rv,Rhs,qnm,qdnm,Dnm]=JacobInd0(tnm,vnm,vdnm,vddnm,Lamnm,unm,...
    par,q0,V,U,B,PJDT,PMDT,PTSDAT,PRSDAT);
Phiqnm=PhiqEval(tnm,qnm,PJDT,par);
QAsLamnm=QAsLamEval(tn,qnm,qdnm,Lamnm,PJDT,par);
J=[Rvdd+(h/2)*Rvd+((h^2)/4)*Rv,Phiqnm'-QAsLamnm];
JCond=cond(J);

%Third derivative calculation, with no explicit time dependence
M=MEval(PMDT,par);
EE=[M*Dnm,Phiqnm'-QAsLamnm];
xx=EE\Rhs;
...
%Solution Estimates for iterative solution
vdd=vddnm+h*vdddnm; 
Lam=Lamnm+h*Lamdnm;
...
R=ResidInd0(tn,v,vd,vdd,Lam,u,B,q0,V,U,...
