function [v,vd,vdd,Lam,ECond,Err,h,nch]=...
...
[vdd,Lam,ECond]=Ind0ODEfunct(tnm,vnm,vdnm,PMDT,PTSDAT,PRSDAT,PJDT,...
    u,q0,V,U,B,par);
k1=vdd;
v2p=vnm+c(2)*h*vdnm;
vd2p=vdnm+h*a1(2,1)*k1;
[vdd,Lam,ECond]=Ind0ODEfunct(tnm+c(2)*h,v2p,vd2p,PMDT,PTSDAT,PRSDAT,PJDT,...
    u,q0,V,U,B,par);
k2=vdd;
v3p=vnm+c(3)*h*vdnm+(h^2)*A(3,1)*k1;
vd3p=vdnm+h*(a1(3,1)*k1+a1(3,2)*k2);
[vdd,Lam,ECond]=Ind0ODEfunct(tnm+c(3)*h,v3p,vd3p,PMDT,PTSDAT,PRSDAT,PJDT,...
    u,q0,V,U,B,par);
k3=vdd;
v4p=vnm+c(4)*h*vdnm+(h^2)*(A(4,1)*k1+A(4,2)*k2);
vd4p=vdnm+h*(a1(4,1)*k1+a1(4,2)*k2+a1(4,3)*k3);
[vdd,Lam,ECond]=Ind0ODEfunct(tnm+c(4)*h,v4p,vd4p,PMDT,PTSDAT,PRSDAT,PJDT,...
    u,q0,V,U,B,par);
k4=vdd;
v5p=vnm+c(5)*h*vdnm+(h^2)*(A(5,1)*k1+A(5,2)*k2+A(5,3)*k3);
vd5p=vdnm+h*(a1(5,1)*k1+a1(5,2)*k2+a1(5,3)*k3+a1(5,4)*k4);
[vdd,Lam,ECond]=Ind0ODEfunct(tnm+c(5)*h,v5p,vd5p,PMDT,PTSDAT,PRSDAT,PJDT,...
    u,q0,V,U,B,par);
k5=vdd;
v6p=vnm+c(6)*h*vdnm+(h^2)*(A(6,1)*k1+A(6,2)*k2+A(6,3)*k3+A(6,4)*k4);
vd6p=vdnm+h*(a1(6,1)*k1+a1(6,2)*k2+a1(6,3)*k3+a1(6,4)*k4+a1(6,5)*k5);
[vdd,Lam,ECond]=Ind0ODEfunct(tnm+c(6)*h,v6p,vd6p,PMDT,PTSDAT,PRSDAT,PJDT,...
    u,q0,V,U,B,par);
k6=vdd;
...
vd=vdnm+h*k*b1';
[vdd,Lam,ECond]=Ind0ODEfunct(tnm+h,v,vd,PMDT,PTSDAT,PRSDAT,PJDT,...
    u,q0,V,U,B,par);
