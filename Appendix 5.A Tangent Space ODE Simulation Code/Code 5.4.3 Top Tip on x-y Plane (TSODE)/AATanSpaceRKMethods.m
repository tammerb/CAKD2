...
%5.3 Top Tip on x-y Plane-Tan Space ODE Solution

intol=10^-6;    %Tolerance in solving discretized equations of motion
Atol=10^-10;     %Absolute error tolerance for variable step methods
utol=10^-12;    %Tolerance in solving for u
Btol=10^-12;    %Tolerance in solving for B
Maxv=10;        %Maximum value of norm(v)
MaxEcond=50;    %Maximum value of cond(E)
Maxiter=6;      %Maximum number of NR iterations in integration
...
Integ=5;    %Numerical Integration Options
...
K=50;
C=0;
omegzpr0=15;      %Initial zprime angular velocity

%Fixed Parameter data List-Complete Partitioning in function parPart
par=[nq;nh;utol;Btol;intol;Atol;m;g;Ixy;Iz;K;C];
...
p0=[1;0;0;0];
q0=[r0;p0];
Omegpr0=[10^-12;10^-12;omegzpr0];
rd0=0*atil(Omegpr0)*uz;
qd0=[rd0;0.5*Geval(p0)'*Omegpr0];
Q(:,1)=q0;
Qd(:,1)=qd0;
qnorm(1)=norm(q0);
qdnorm(1)=norm(qd0);
...
if Integ==5
[vn,vdn,vddn,jiter,R1Norm,JCond,J]=ImplicitTrap(n,tn,...
...
[vn,vdn,vddn,jiter,R1Norm,JCond,h,nch,R1,k1,k,J]=ImplicitSDIRK54(n,tn,...
    Vv,Vvd,Vvdd,Uu,Q,Qd,Qdd,q0,U,V,B,par,intol,Atol,nq,nh,h,...
    hmax,nch,hvar,npar,k1); 
hrpt(n)=h;
R1normRpt(n)=R1Norm;
RR1(:,n)=R1;
...
M=Meval(qn,par);
KE(n)=0.5*qdn'*M*qdn;
TE(n)=KE(n)+m*g*rz(n)+0.5*K*rt'*rt;

%Calculagtion of norm of zpr angular momentum
Jpr=diag([Ixy;Ixy;Iz]);
...
qd=Qd(:,n);
qdd=Qdd(:,n);
PosConstrNorm(n) = norm(Phi(t,q,par));
Phiq = P1(t,q,par);
